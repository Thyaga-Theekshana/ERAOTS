...
export const eventsAPI = {
  ...
  employeeStates: (status) =>
    api.get("/api/events/occupancy/employees", {
      params: { status_filter: status },
    }),
  // Status override - manual toggle between ACTIVE/IN_MEETING
  statusOverride: (newStatus) =>
    api.put("/api/events/status-override", null, {
      params: { new_status: newStatus },
    }),
  // Pending transitions (meeting confirmations)
  getPendingTransitions: () => api.get("/api/events/pending-transitions"),
  confirmTransition: (transitionId) =>
    api.put(`/api/events/pending-transitions/${transitionId}/action`, null, {
  ...
  statusTimeline: (employeeId, date) =>
    api.get(`/api/events/status-timeline/${employeeId}`, {
      params: { target_date: date },
    }),
  // Calendar integration
  getCalendarSettings: () => api.get("/api/events/calendar-settings"),
  updateCalendarSettings: (data) =>
    api.put("/api/events/calendar-settings", data),
  // Important meetings
  listSpecialMeetings: () => api.get("/api/events/special-meetings"),
  createSpecialMeeting: (data) =>
    api.post("/api/events/special-meetings", data),
  triggerSpecialMeeting: (meetingId) =>
    api.post(`/api/events/special-meetings/${meetingId}/trigger`),
};

// ==================== ATTENDANCE ====================
export const attendanceAPI = {
  process: (targetDate) =>
    api.post("/api/attendance/process", null, {
      params: { target_date: targetDate },
    }),
  list: (params) => api.get("/api/attendance/", { params }),
  myInsights: (days = 30) =>
    api.get("/api/attendance/my-insights", { params: { days } }),
  teamInsights: (days = 30, departmentId = null) =>
    api.get("/api/attendance/team-insights", {
      params: { days, department_id: departmentId },
    }),
  companyInsights: (days = 30) =>
    api.get("/api/attendance/company-insights", { params: { days } }),
};

// ==================== SYSTEM (SUPER_ADMIN) ====================
export const systemAPI = {
  insights: (days = 7) =>
    api.get("/api/events/system-insights", { params: { days } }),
};
...
export const leaveAPI = {
  ...
  listRequests: (status) =>
    api.get("/api/schedules/leave-requests", { params: { status } }),
  myRequests: () => api.get("/api/schedules/leave-requests/my"),
  getUsage: (year) =>
    api.get("/api/schedules/leave-usage", { params: { year } }),
  getBalance: (year) =>
    api.get("/api/schedules/leave-balance", { params: { year } }),
  getCalendar: (month) =>
  ...
  updateStatus: (id, status_val, comment) =>
    api.put(`/api/schedules/leave-requests/${id}/status`, null, {
      params: { status: status_val, comment },
    }),
};
...
export const correctionsAPI = {
  ...
  myCorrections: (status) =>
    api.get("/api/corrections/my", { params: { status } }),
  updateStatus: (id, status_val, comment) =>
    api.put(`/api/corrections/${id}/status`, null, {
      params: { status: status_val, comment },
    }),
  managerApprove: (id, comment) =>
    api.put(`/api/corrections/${id}/manager-approve`, null, {
      params: { comment },
    }),
  managerReject: (id, comment) =>
    api.put(`/api/corrections/${id}/manager-reject`, null, {
      params: { comment },
    }),
  hrApprove: (id, comment) =>
    api.put(`/api/corrections/${id}/hr-approve`, null, {
      params: { comment },
    }),
  hrReject: (id, comment) =>
    api.put(`/api/corrections/${id}/hr-reject`, null, {
      params: { comment },
    }),
};
...
export const emergencyAPI = {
  ...
  markAccounted: (headcountId) =>
    api.put(`/api/emergency/headcount/${headcountId}/account`),
  // Safety Check
  sendSafetyCheck: (emergencyId, data = {}) =>
    api.post(`/api/emergency/${emergencyId}/safety-check`, data),
  getSafetyCheck: (emergencyId, statusFilter) =>
    api.get(`/api/emergency/${emergencyId}/safety-check`, {
      params: { status_filter: statusFilter },
    }),
  respondSafetyCheck: (response) =>
    api.put("/api/emergency/safety-check/respond", { response }),
};
...
export const productivityAPI = {
  ...
  getTeamStats: (date) =>
    api.get("/api/productivity/team-stats", { params: { date } }),
};
...
export const hardwareAPI = {
  ...
  getHealthHistory: (scannerId) =>
    api.get(`/api/scanners/${scannerId}/health-history`),
  register: (data) => api.post("/api/scanners", data),
  list: () => api.get("/api/scanners"),
  sendHeartbeat: (scannerId, data) =>
    api.post(`/api/scanners/${scannerId}/heartbeat`, data),
  syncBuffer: (scannerId, data) =>
    api.post(`/api/scanners/${scannerId}/buffer-sync`, data),
